...
- CPU worker benchmark (`normals`/`ao` rows) hasn't been re-measured since A11 (2026-09-18) dropped `K` for a full-radius covariance and raised the default radius to 6×: the shipped algorithm and radius no longer match the CPU figures in § Compute › Measured (pre-4b baseline table). Owner: any bench-focused task.
...
`normalsKernel` (radius PCA, amended by A11, 2026-09-18 — replaces the `K = 16` register kNN below): 27-cell neighbourhood (`c0 ± 1`, skipping negative cells), single-pass **centred covariance over every neighbour within `radius`** — no candidate cap, no sort. Per point: `n` starts at 1 (the point itself, `d = 0`); for each neighbour, `d = p_j − p_i` accumulates into `n`, `s = Σd`, and the six unique terms of `Σddᵀ`. Centring on `p_i` rather than a neighbourhood centroid keeps every accumulated term bounded by `radius` regardless of where the patch sits in the (large, bounds-relative) coordinate space, so the f32 WGSL sums stay exact enough to match the f64 CPU mirror (`compute/cpu/normals.ts`, same accumulation order) without a first pass to find a centroid. `n < 4` (fewer than 3 real neighbours) is degenerate → the +Z word; otherwise `cov = Σddᵀ/n − mmᵀ` (`m = s/n`) feeds the unchanged 3×3 cyclic Jacobi (8 sweeps, Numerical Recipes rotation, identical in WGSL and TS), oriented to `nz ≥ 0`, oct-encoded as two **unsigned** u16 (`round((p·0.5+0.5)·65535)`, decoded `/65535` on both sides — controller ruling `bb959f2`). Degenerate word is `0x80008000` (WGSL `round(32767.5)` → 32768 half-to-even; in JS the constant must be written unsigned, `(32768 | 32768 << 16) >>> 0`, or it compares as a negative int32 and never matches a `Uint32Array` element). `aoKernel` is unchanged by A11 — it was already radius-based, never `k`-capped: per point, `1 − above/total` over neighbours within the radius, "above" = `d·n > eps` with `eps = 0.02 × radius`; `total = 0` → 1; byte = `round(a·255)`. Readback sanity, re-verified after the rewrite (2M, radius 3×): **+Z 117,789 / 2,000,000**, next to the pre-A11 `K = 16` estimator's 117,790 — ground behaviour is unaffected by dropping the cap, as expected (ground neighbourhoods were never near the old 16-neighbour limit).

### Normal quality (phase 4b)

Root cause: at 20M density (spacing ≈ 0.224 m) the pre-4b `K = 16` cap sat within ~0.5 m of any point regardless of `radius` — the slider grew the *search* neighbourhood but the kept sample never grew past 16, so widening it did nothing for facade normal quality (the plan's diagnosis). Tracked with the DEV `classStats()` hook (`ComputeRunner.tsx`): a per-class histogram of `|n.z|` (10 bins, oct-decoded from a `readback()`) plus mean AO, keyed by `qpos`'s classification byte (`word1 >>> 24`); the **wall bin** `[0, 0.1)` on class 6 (building) is the fraction of facade points whose normal reads near-horizontal — the direct measure of the "flat smear, no wall peak" symptom.

| config | class-6 wall bin, 2M | class-6 wall bin, 20M |
|---|---|---|
| baseline (`K = 16`, 3× = 2.12 m / 0.67 m) | 0.043 | not independently measured (qualitatively flat per the plan's diagnosis) |
| radius PCA, 3× (2.12 m / 0.67 m) | 0.043 | 0.021 |
| radius PCA, 6× (4.24 m / 1.34 m — the 4b default) | 0.109 | 0.078 |

Dropping the `K` cap alone leaves the 2M wall bin unchanged (0.043 → 0.043 at the same 3× radius) — the demo's ~30-neighbour disc at 3× was never `K`-limited, so the cap's effect is invisible there; no 20M baseline was measured under `K = 16` to run the same check. What is measured: widening the default from 3× to 6× (radius PCA both times) raises the wall bin 2.5× at 2M (0.043 → 0.109) and 3.7× at 20M (0.021 → 0.078). Ground (class 2) stays clean throughout: baseline 97.2 % top bin (`|n.z| > 0.9`) → 97.5 % (radius PCA, 3×, 2M) → ≈99 % (radius PCA, 6×, 20M). A smoothing pass — a second compute pass averaging each point's normal with its neighbours', costing a second `normals`-sized buffer (+80 MB at 20M) — was designed and costed, then dropped: radius PCA plus the 6× default alone clears the acceptance bar (≥ 0.10 @2M, ≥ 0.07 @20M) at normals/AO costs of 34–36/46 ms (2M) and 630/685 ms (20M), so the extra pass and buffer bought nothing the radius change didn't already deliver (`docs/superpowers/plans/2026-09-18-phase-4b-normal-quality.md`, Task 2). Memory is unchanged from the Phase 4 table below — no smoothing buffer was added.

### Shading (`render/pointMaterial.ts`)

`shading` uniform (`flat 0 / lit 1 / litAo 2 / normals 3`, the last a debug view). The vertex stage oct-decodes `normalsNode[gi]`, transforms with `transformNormalToView` for the view-space term, and lights with wrap + fixed sun (A11): `wrap = 0.30 + 0.45·|n_view · v|` (never fully black, camera-facing flip via `abs`) plus `sun = 0.25·max(n_world · L, 0)` for a fixed key light `L = normalize(-0.4, -0.3, 0.85)` (world +Z up; `normalObj` doubles as `n_world` — positions are centred, model matrix identity) — `lambert = wrap + sun` (0.30…1.0). AO byte from `aoNode[gi >> 2]` applies as `sqrt(ao)`, so occlusion shades rather than masking to black. The four modes are one **branchless** blend — `lit = step(0.5, shading)`, `useAo = step(1.5, shading)`, `debugNormals = step(2.5, shading)`, `light = mix(1, lambert · mix(1, sqrt(ao), useAo), lit)` — and `colorNode = mix(lut × vertexStage(light), vertexStage(|normalObj|), debugNormals)`, so the storage reads and the lighting evaluate once per vertex. The nested-`select` version rendered nothing in flat/litAo (three-0.186 finding recorded in the Phase 0 rules below). `lambert` is evaluated in flat mode too; pre-build all-zero normal words decode to `(0,0,−1)`, no NaN. Frame cost unchanged from the prior headlight formula — same vertex-stage op count, no new branches: 2M `4.17 ms` in flat, lit and lit + AO (vsync floor, reverified at radius 6× default); 20M `34–37 ms` before the build, `34.7–35.7 ms` after, `35 ms` in lit + AO — `draws 257 tris 40000001` unchanged.
...
Same algorithms in TS over typed arrays (`decodePositions` + `buildGrid`, `computeNormals`, `computeAo`; vitest: `forEachNeighbour` (all neighbours within radius) against a brute-force distance scan — A11 dropped the separate `knn` describe block — Jacobi on known matrices, oct round-trip < 0.5° (test bound; measured worst over the test's 2,000 PRNG vectors 0.0035° at [0.698, −0.427, −0.575], +Z 0.0012°), exclusive scan vs `reduce`, plane AO = 1, corner AO 0.68, a noisy-facade normal within 10° (radius PCA regression test, A11) — 27 tests across `compute/params.test.ts`, `compute/cpu/*.test.ts`, `compute/verify.test.ts`). Runs in the existing loader worker (ruling 4: `cpuBench`/`cancelBench` on `LoaderIn`, `benchProgress`/`benchDone`/`benchCancelled` on `LoaderOut`), cooperative in 100k-point slices with a `setTimeout(0)` yield so a cancel lands between slices; results transferred back. Input = `benchWords(qpos.array, chunks, min(N, BENCH_CAP))`: the whole array when `N ≤ 2M` (then `slice()`d so the attribute's own buffer is never transferred), else the first `floor(n/chunks)` points of every chunk packed contiguously without padding — 256 × 7,812 = **1,999,872** points on the full set. The bench posts `store.bench`; DEV hook `window.__pcvBench = { state, run }` (separate from `window.__pcvCompute = { build, readback, tableSize, timings, state, cpuCellStart, classStats }`, owned by `<ComputeRunner>`'s effect). `classStats()` (added with the `normals` debug shading mode, A11) awaits `readback()` and rebuilds the per-class `|n.z|` histogram + mean AO behind § Normal quality's numbers. `readback()` throws before the first build (`getArrayBufferAsync` on an attribute no pipeline has bound). Cancel verified: Cancel ~3 s in → `status 'cancelled', progress 0.43`, re-run works.

### Verify (`compute/verify.ts`)

`compareResults(gpuNormals, gpuAo (packed words), cpuNormals, cpuAo, n)` → sign-insensitive angle per point (`acos(|a·b|)`), median/max, `+Z` count on the GPU side, AO MAE normalised to [0,1], non-finite count. Enabled only when `pointCount ≤ BENCH_CAP` (same point set on both sides); on the full set the panel shows "Verify needs the same points on both sides — demo set only." Pre-4b result (`K = 16`, 2M, radius 3×): `n 2,000,000 · median 0.000° · max 80.77° · AO MAE 0.0001 · non-finite 0 · +Z 117,790`; tail ≤ 0.1° 1,999,570 · (0.1, 1]° 217 · (1, 5]° 123 · (5, 20]° 76 · > 20° 14 (fraction > 1° = 1.07e-4), all on near-isotropic neighbourhoods where f32 (WGSL) vs f64 (JS) Jacobi picks a different smallest eigenvector, or a distance tie at the `K = 16` cut resolving differently between the GPU's atomic scatter order and the CPU's index-ascending order. AO: 52,676 of 2M bytes differ, 51,808 by exactly 1 LSB (WGSL `round()` is ties-to-even, `Math.round` is half-up), 868 by more (points whose normal differs). **Radius PCA result (A11, 2M, radius 3×):** `n 2,000,000 · median 0.000° · max 81.23° · AO MAE 0.0001 · non-finite 0 · +Z 117,789` — the `K`-cut tie source is gone (GPU and CPU now sum the identical neighbour set within `radius`, no top-`k` selection to disagree on), so the residual disagreement is f32-vs-f64 Jacobi alone; median and AO MAE unchanged, max moved within the same near-isotropic-neighbourhood noise band. Verify reports MAE with no gate; the median is the acceptance number.

### Measured

**Pre-4b baseline** (`K = 16`, radius 3 × spacing = 2.12 m demo / 0.67 m full — kept for reference, superseded by A11 below). GPU = timestamp query per pass; console 0 errors in every run (2 benign warnings on the demo, 3 on the full set — `PCFSoftShadowMap` fires once per StrictMode mount).
...
**4b default** (radius PCA, A11, 2026-09-18 — 6 × spacing = 4.24 m demo / 1.34 m full; the shipped algorithm and radius):
...
- 2M target < 200 ms GPU: comfortably met at every stage — pre-4b baseline 87–93 ms (`K = 16`, 3×) → 59 ms (radius PCA, same 3×, Task 1) → 80–83 ms (radius PCA, 6× default): dropping `K` saves more than doubling the default radius costs back. First-ever build after a reload includes pipeline compile (Task 5, pre-4b: normals 56.75, ao 36.70, wall 208 ms; Task 4 hash-only: 181.8 ms cold → 20.1 ms warm).
- 20M, radius PCA vs `K = 16` at the *same* 3× radius: `normals` 818 → 743 ms, `ao` 704 → 667 ms — both cheaper with the 16-slot register insertion sort gone (likely spilled on Metal, per the removed Deferred entry). Moving the default to 6× then costs `normals` less again (743 → 630 ms — more neighbours accumulated per point, but still cheaper than the old capped-and-sorted kernel) and `ao` a little more (667 → 685 ms, more candidates to scan at the wider radius); build total 1,321 ms, wall 2,536 ms. `T = 4,194,304`; `builtRadius` 0.6708 m (3×) / 1.3416 m (6×).
...
- CPU worker benchmark (`normals`/`ao`) hasn't been re-measured under radius PCA: the CPU columns in the baseline table above are `K = 16`-era and no longer describe the shipped algorithm or the 6× default (Deferred).
