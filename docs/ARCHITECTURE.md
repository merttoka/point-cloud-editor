...
- The lasso kernel is 1.0–1.2 ms at 20M (thread per word, 5M threads) — the 20 MB readback (135–173 ms) and the CPU bookkeeping (undo copy 20 MB, `countFlags` + `selectionRange` ≈ 2 × 20M-byte passes) are the cost; see Deferred for why the partial-readback fallback was not taken.
