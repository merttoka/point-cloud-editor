...
**Hosting / embedding**
- Decided in phase 6 (§ Bench and deploy): the app is its own Vercel project and fetches both datasets at build time, so the browser never touches the CORS-less release URLs. The Lab proxies `/point-cloud/app/*` to it, same-origin. Live since 2026-09-26; `check_hosting.py --no-cors` passes on the project URL and through the Lab proxy (§ Bench and deploy › Hosting). Remaining gap: the data is served `immutable` at unversioned paths, so a data change needs a new folder or release tag (owner: any).
...
- `check_hosting.py --no-cors http://localhost:4173/point-cloud/app/data/demo/points.bin` (`vite preview`): `range 206` PASS, `content-range` PASS (`bytes 0-15/16000000`, 16 B body), `accept-ranges` PASS.
- Production, 2026-09-26, `…/data/full/points.bin` on `point-cloud-editor.vercel.app` and on `lab.merttoka.com` (through the proxy): both `range 206` PASS, `content-range` `bytes 0-15/160000000`, `accept-ranges: bytes`; the manifest carries `cache-control: public, max-age=31536000, immutable` and `access-control-allow-origin: https://lab.merttoka.com` on both origins; `/point-cloud/app` 308s to the slash form. One deploy fix was needed: the bare directory URL rewritten to `/` returned NOT_FOUND, so `vercel.json` now rewrites `/point-cloud/app` and `/point-cloud/app/` to `/index.html` explicitly and files through `:path+`.
