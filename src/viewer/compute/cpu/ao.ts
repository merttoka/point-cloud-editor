...
export function computeAo(g: Grid, pos: Float32Array, normals: Uint32Array, end: number, eps: number, out = new Uint8Array(end), start = 0): Uint8Array {
  for (let i = start; i < end; i++) out[i] = Math.round(aoAt(g, pos, i, octDecode(normals[i]), eps) * 255)
  return out
}
